...
# Fix Mojibake everywhere else in the file if present
text = text.replace('A ligar a:', 'A ligar a:')
text = text.replace('Ã°Å¸â€ Å\xa0', '🎧')
text = text.replace('Erro ao reproduzir ', 'Erro ao reproduzir ')
text = text.replace('Erro na emissÃƒÂ£o', 'Erro na emissão')
text = text.replace('Erro ao iniciar rÃƒÂ¡dio.', 'Erro ao iniciar rádio.')
